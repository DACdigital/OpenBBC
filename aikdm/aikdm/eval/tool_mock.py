...
def _synthesize_from_schema(shape: Any) -> Any:
    ...
    an empty dict when shape is None or unrecognised. Not a full JSON
    Schema faker — just enough to keep the tested agent moving forward."""
    ...
    if t == "array":
        ...
        return [_synthesize_from_schema(item)] if item is not None else []
    if t == "string":
        return "example"
    if t == "integer":
        return 0
