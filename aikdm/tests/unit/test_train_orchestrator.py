...
def test_failed_propose_patches_doesnt_kill_loop(monkeypatch):
    scores = iter([0.5])
    async def fake_run_eval(inp, settings):     # noqa: ARG001
        return _result(next(scores))

    async def fake_propose(*, agent, bundle, eval_result, tried):    # noqa: ARG001
        raise RuntimeError("teacher LLM timed out")
    ...
    assert report.total_epochs_run == 2
    for ep in report.epochs:
        assert ep.error.startswith("propose_patches:")
        assert ep.promoted is False
    # Baseline preserved.
    assert report.final_score == 0.5


def test_stops_when_baseline_already_perfect(monkeypatch):
    async def fake_run_eval(inp, settings):     # noqa: ARG001
        return _result(1.0)  # baseline is already perfect

    async def fake_propose(*, agent, bundle, eval_result, tried):   # noqa: ARG001
        raise AssertionError("teacher should not be called when baseline is perfect")
    ...
    assert report.total_epochs_run == 0
    assert report.initial_score == 1.0
    assert report.final_score == 1.0


def test_stops_when_promoted_candidate_hits_perfect_score(monkeypatch):
    # baseline=0.5, candidate=1.0 — should stop after epoch 1 with perfect_score.
    scores = iter([0.5, 1.0])
    async def fake_run_eval(inp, settings):     # noqa: ARG001
        return _result(next(scores))
    ...
    async def fake_propose(*, agent, bundle, eval_result, tried):   # noqa: ARG001
        return teacher_mod.TeacherTurn(
            output=TeacherOutput(patches=[
                SectionPatch(section_id="main_prompt", new="perfected", rationale="x"),
            ]),
            tokens_in=1, tokens_out=1,
        )
